...
- **Custom code** = the app repo: `app/` (Models, Http/Controllers, Services/Actions, Jobs, Providers) + `packages/` if existing, with the app structure split by section, `routes/`, `database/migrations`. Build/orchestration (`composer.json`, `artisan`, CI) → containers (C2).
